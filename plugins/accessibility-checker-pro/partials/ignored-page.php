...
<div class="wrap edacp-ignored">
	...
	$ignore_tab_items = array(
		array(
			'slug'  => '',
			'label' => esc_html__( 'Individually Ignored Issues', 'edacp' ),
			'order' => 1,
		),
		array(
			'slug'  => 'global',
			'label' => esc_html__( 'Global Ignores', 'edacp' ),
			'order' => 2,
		),
	);
	...
	<?php if ( $ignore_tab_items ) : ?>
		<nav class="nav-tab-wrapper">
			<?php foreach ( $ignore_tab_items as $settings_tab_item ) : ?>
				...
				?>
				<a
					href="?page=accessibility_checker_ignored<?php echo esc_html( $query_var ); ?>"
