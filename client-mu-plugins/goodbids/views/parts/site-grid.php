	...
	<a class="flex items-center gap-4 no-underline hover:underline focus:underline" href="<?php echo is_admin() ? '#' : esc_url( get_site_url() ); ?>">
		<?php if ( $image_attributes ) : ?>
			<div class="w-10 h-10 overflow-hidden rounded-full shrink-0">
				<img class="object-contain w-10 h-10" src="<?php echo esc_url( $image_attributes[0] ); ?>" width="<?php echo esc_attr( $image_attributes[1] ); ?>" height="<?php echo esc_attr( $image_attributes[2] ); ?>" alt="<?php echo esc_attr( get_bloginfo( 'title' ) ); ?> logo" />
			</div>
		<?php endif; ?>
